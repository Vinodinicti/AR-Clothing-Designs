...
function renderHomePageDesigns() {
    ...
    if (!designGrid || !PRODUCTS_DATA || PRODUCTS_DATA.length === 0) return;
        ...
        return `
            <div class="design-card" data-id="${escapeHtml(item.id)}">
                ...
                <div class="design-info">
                    <p class="design-category">${escapeHtml(itemCategory)}</p>
    ...
    attachCardEventListeners(designGrid);
}

function renderDesignsPage() {
    const collectionGrid = document.querySelector(".collection-grid");
    if (!collectionGrid || !PRODUCTS_DATA || PRODUCTS_DATA.length === 0) return;

    const activeFilterBtn = document.querySelector(".filter-btn.active");
    const activeCategory = activeFilterBtn ? activeFilterBtn.getAttribute("data-filter") : "all";

    collectionGrid.innerHTML = PRODUCTS_DATA.map(item => {
        const itemCategory = item.category || 'men';
        const isVisible = (activeCategory === "all" || itemCategory === activeCategory);
        const tagHtml = item.tag ? `<span class="collection-tag">${escapeHtml(item.tag)}</span>` : '';
        const sizesArr = item.sizes || ["S", "M", "L", "XL"];
        ...
        return `
            <div class="collection-card ${isVisible ? 'show' : ''}" data-category="${escapeHtml(itemCategory)}" data-id="${escapeHtml(item.id)}" style="${isVisible ? '' : 'display: none;'}">
                <div class="collection-image">
                    ...
                    ${tagHtml}
                    <button class="wishlist-btn-card ${isWishlisted ? 'active' : ''}" data-id="${escapeHtml(item.id)}" aria-label="Add to Wishlist" title="${isWishlisted ? 'Remove from Wishlist' : 'Add to Wishlist'}">
                        <svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
                    </button>
                </div>
    ...
    attachCardEventListeners(collectionGrid);
}

function renderProducts() {
    const productGrid = document.querySelector(".product-grid");
    if (!productGrid || !PRODUCTS_DATA || PRODUCTS_DATA.length === 0) return;

    const activeCatBtn = document.querySelector(".category-btn.active");
    const activeCategory = activeCatBtn ? activeCatBtn.getAttribute("data-category") : "all";

    productGrid.innerHTML = PRODUCTS_DATA.map(item => {
        const itemCategory = item.category || 'men';
        const isVisible = (activeCategory === "all" || activeCategory === itemCategory || (item.categoryLabel && item.categoryLabel.toLowerCase().includes(activeCategory)));
        const sizesArr = item.sizes || ["S", "M", "L", "XL"];
        const isWishlisted = wishlist ? wishlist.includes(item.id) : false;

        return `
            <div class="product-card ${isVisible ? 'show' : ''}" data-category="${escapeHtml(itemCategory)}" data-id="${escapeHtml(item.id)}" style="${isVisible ? '' : 'display: none;'}">
                ...
                <div class="product-info">
                    <p class="product-category">${escapeHtml((item.categoryLabel || itemCategory).toUpperCase())}</p>
                    <h3>${escapeHtml(item.name)}</h3>
                    <p class="product-description">${escapeHtml(item.description || '')}</p>
                    <div class="product-details">
                        <span class="price">₹${parseFloat(item.price).toLocaleString('en-IN')}</span>
                    </div>
                    <p class="product-size">Sizes: ${sizesArr.join(' • ')}</p>
                    <div class="card-actions-row">
                        <button class="btn-card-cart add-cart-btn" data-id="${escapeHtml(item.id)}">Add to Cart</button>
                        <button class="btn-card-buy buy-now-btn" data-id="${escapeHtml(item.id)}">Buy Now</button>
                    </div>
    ...
    attachCardEventListeners(productGrid);
}

document.addEventListener('DOMContentLoaded', fetchLiveProducts);
...
function addToCart(productId, size = "M", quantity = 1) {
    const product = PRODUCTS_DATA.find(p => String(p.id) === String(productId) || p.name === productId || p.title === productId);
    if (!product) return;
    ...
    // Live sync to backend SQLite database
    fetch('/api/cart', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            user_email: localStorage.getItem('ar_user_email') || 'customer@arclothing.com',
            user_name: localStorage.getItem('ar_user_name') || 'Guest Customer',
    ...
    }).catch(e => console.error('Live cart sync error:', e));
...
function toggleWishlist(productId) {
    ...
    const product = PRODUCTS_DATA.find(p => String(p.id) === String(productId) || p.name === productId || p.title === productId);
    const title = product ? (product.name || product.title) : "Design";
        ...
        if (product) {
            fetch('/api/likes', {
                ...
                body: JSON.stringify({
                    ...
                    product_name: product.name,
                    product_id: product.id,
                    category: product.categoryLabel || 'Clothing',
                    price: product.price
                })
            }).catch(e => console.error('Live wishlist sync error:', e));
        }
    }
...
async function submitOrderToBackend() {
    ...
    const name = document.getElementById("shipName")?.value || "Aarav Sharma";
    const email = document.getElementById("shipEmail")?.value || "aarav.sharma@example.in";
    const phone = document.getElementById("shipPhone")?.value || "+91 98765 43210";
    const address = document.getElementById("shipAddress")?.value || "12 MG Road";
    const city = document.getElementById("shipCity")?.value || "Mumbai";
    const pin = document.getElementById("shipPin")?.value || "400001";
    const fullAddress = `${address}, ${city} - ${pin}`;

    // 1. Sync / Save Customer Profile in Users Table
    try {
        ...
        console.error('Error saving customer profile to backend:', err);
    }

    // 2. Submit each ordered item to Orders Table
...
function processPayment() {
    ...
    // Persist ordered products to backend SQLite orders table
